...
import { FacultyFormDialog } from "~/components/dialogs/faculty-form-dialog";
...
export default function AdminFacultiesPage() {
  const [faculties, setFaculties] = useState<UserSchema[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [search, setSearch] = useState<string>("");
  const [open, setOpen] = useState<boolean>(false);

  const filteredFaculties = faculties.filter((t) => {
  ...
  useEffect(() => {
    const unsubscribe = getUsersRealtime({ role: "faculty" })((v) => {
      setFaculties(v);
      setLoading(false);
    });
  ...
  return (
    <AdminLayout className="gap-4 p-4">
      <div className="flex h-16 items-center justify-between">
        <span className="text-xl font-medium">Faculties</span>
                ...
                <TableRow key={faculty.id}>
                  <TableCell>
                    <Avatar className="size-12">
                      <AvatarImage
                        src={faculty.profile}
                        alt={faculty.firstName[0].toUpperCase()}
                      />
                  ...
                  <TableCell className="uppercase">{faculty.status}</TableCell>
                  <TableCell className="uppercase">
                    {format(faculty.dateCreated, "MMM dd, yyyy")}
                  </TableCell>
                  <TableCell>
      ...
      <FacultyFormDialog open={open} onOpenChange={(v) => setOpen(v)} />
    </AdminLayout>
  );
}
