...
interface TalentDetailsFacultyControlsProps {
  talent: TalentSchema;
}

function TalentDetailsFacultyControls(
  props: TalentDetailsFacultyControlsProps,
) {
  const { talent } = props;
  ...
  const { component, openBoolean } = useBoolean({
    ...
    onPositive: async () => {
      ...
      router.replace(`/faculty/${talentType}`);
    },
  });
...
export { TalentDetailsFacultyControls };
